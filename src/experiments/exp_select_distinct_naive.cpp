...
int main(int argc, char** argv) {
  ...
  **/
  ...
  BShare *selected = (BShare *) malloc(ROWS*sizeof(BShare));
  assert(selected !=NULL);

  // copy selected bits from t1 into selected
  for (int i=0; i<ROWS; i++) {
    selected[i] = t1.content[i][2];
  }

  distinct_linear(&t1, sort_att, 1, selected);
  ...
  open_b_array(selected, ROWS, open_res);  
  ...
  if (rank == 0) {
    printf("%d\tNAIVE select-distinct\t%.3f\n", ROWS, elapsed);
  }

  free(selected); free(open_res);
