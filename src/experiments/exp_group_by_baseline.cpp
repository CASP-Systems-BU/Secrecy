...
#define COLS 2 // counter shares are stored in the last 2 columns

/**
 * Evaluates the performance of baseline group-by-count (with RCA).
 * We use this experiment to show the benefits of
 * share bimodality (binary and arithmetic).
 **/
...
int main(int argc, char** argv) {
  ...
  // initialize counters and selected
  for (int i=0; i<ROWS; i++) {
    t1.content[i][2] = (rank % 2) & 1;
    t1.content[i][3] = (succ % 2) & 1;
  }

  /* =======================================================
     1. Measure RCA group-by
  ======================================================== */
  // start timer
  gettimeofday(&begin, 0);

  unsigned key_index[1] = {0};
  group_by_sum_rca_odd_even(&t1, BATCH_SIZE_, key_index, 1);
