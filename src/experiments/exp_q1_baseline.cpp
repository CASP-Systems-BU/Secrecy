...
int main(int argc, char** argv) {
  ...
  assert(in_res!=NULL);
  BShare *in_res_rem = (BShare *) malloc(ROWS1*sizeof(BShare));
  ...
  for (int i=0; i<ROWS1; i++) {
    t1.content[i][6] = in_res[i];
    t1.content[i][7] = in_res_rem[i];
  }
  ...
  #endif
  unsigned key_index[1] = {2};
  // NOTE (john): This function does not properly mask unselected rows, i.e., 
  // rows with zero count, but it has the exactly same cost, which is OK for 
  // this experiment
