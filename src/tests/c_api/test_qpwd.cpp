...
int main(int argc, char** argv) {
  ...
  assert(rand_b!=NULL);
  AShare *rand_a = (BShare *) malloc(num_rands*sizeof(AShare));
  assert(rand_a!=NULL);
  ...
  for (int i=0; i<ROWS1; i++)
    selected_a[i] = sel_a;
  group_by_count_sel_odd_even(&t1, key_indices, 2, 3, selected_b, selected_a, rand_b, rand_a);
  ...
  for (int i=0; i<ROWS1; i++) {
    ...
    t1.content[i][5] = selected_b[i];  // Remote share of count
  }
